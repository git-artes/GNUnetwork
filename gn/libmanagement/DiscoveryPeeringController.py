        ...
        while not self.finished :
            ...
            if event.ev_type == "Timer":
                ...
                if event.ev_type == "Mgmt":        
                    if (event.ev_dc['dst_addr'] == self.my_addr or event.ev_dc['dst_addr'] == self.broadcast_addr) and event.ev_dc['src_addr'] != self.my_addr :
                        "In this case the destination mac is my mac or is a broadcast packet and I am not the source" 
                        if self.my_peers.isMember(peerMACaddr = event.ev_dc['src_addr']):
                            "In this case the peer link exists in the peers table"
                            #print "Evento 1: ", event,'  my mac: ',self.my_addr, '\n'
                            #self.my_peers.printPeersTable()                
                            sm= self.my_peers.getSM(peerMACaddr= event.ev_dc['src_addr'])
                            self.my_peers.updatePeerLinkId(event.ev_dc['src_addr'], event.ev_dc['peerlinkId'])
                            ...
                            self.my_peers.add(event.ev_dc['src_addr'], self.my_queue, self.tx_event_q, self.net_conf, self.local_linkId, event. ev_dc['peerlinkId'])
                        ...
                        print "Error: wrong MAC address, destination address:",  event.ev_dc['dst_addr'], " source address : ", event.ev_dc['src_addr']," my address : ", self.my_addr               
    
                else: 
                    print "Error: wrong event type"
                    
    def moveFSM(self,event,sm):
        if event.nickname == "MgmtBeacon" and self.net_conf.accepting_additional_peerings :
